...
const COLORS = ["#F2D4B5", "#C98266", "#6E3C3C", "#79C9D2", "#E1E6E7", "#BCBBB5"];
const ITEMS = [cupPNG, tissuePNG, duckPNG];
const ENCOURAGEMENTS = ["LOOKIN' FINE!", "GOOD CHOICE!", "NICE PICK!", "LOVE IT!"];
const STATBONUS = ["NONE", "+3 ENERGY", "+3 MORALE", "+3 VIBES"];

function ColorPicker({ selectedSkin, setSelectedSkin, setEncouragement } : {selectedSkin: number, setSelectedSkin: (skin: number) => void,setEncouragement: React.Dispatch<React.SetStateAction<number>>}) {
  return (
...
function ItemPicker({ selectedItem, setSelectedItem }: { selectedItem: number, setSelectedItem: (item: number)=> void }) {
  return (
...
function SignDisplay({ page, encouragement, selectedItem }: {page: number, encouragement: number, selectedItem: number}) {
  return (
...
export default function PlayerSelect() {
  ...
  const { selectedItem, selectedSkin, setSelectedItem, setSelectedSkin } = useContext(Context);
  ...
  return (
    <div className="overflow-hidden bg-linear-to-b from-[#ACDCFD] via-[#B3E9FC] to-[#B9F2FC] h-[100vh] w-full">
      <img src={tree2SVG} alt="Tree" className="sm:block hidden absolute h-[300px] w-[300px] bottom-[80px] left-[25px]" />
      <img src={shrub_flowerSVG} alt="Shrubs" className="sm:block hidden absolute h-[90px] w-[200px] bottom-[90px] left-[450px]" />
      <img src={birdSVG} alt="Bird" className="sm:block hidden absolute h-[90px] w-[200px] bottom-[90px] left-[300px]" />
      <img src={shrub_flowerSVG} alt="Shrubs" className="sm:block hidden absolute h-[90px] w-[200px] bottom-[90px] right-[150px]" />
      ...
      <div className="w-full h-full flex items-center justify-center px-4 py-8 overflow-hidden">
        <div className="flex flex-col sm:flex-row items-center sm:items-start justify-center gap-12 sm:gap-[150px] w-full max-w-[1200px]">
          ...
          <div className="flex-col items-center sm:items-end w-full sm:w-[430px] gap-4 sm:flex hidden">
            ...
            {page === 2 ? (
              <ItemPicker selectedItem={selectedItem} setSelectedItem={setSelectedItem} />
            ) : (
              <ColorPicker selectedSkin={selectedSkin} setSelectedSkin={setSelectedSkin} setEncouragement={setEncouragement} />
            )}
          ...
          <div className="flex flex-col items-start gap-4">
            <div className="relative">
              <img src={frameSVG} alt="Character Frame" className="h-[290px] w-[290px] sm:h-[420px] sm:w-[420px]" />
              <img
            ...
            <SignDisplay page={page} encouragement={encouragement} selectedItem={selectedItem} />
          </div>
